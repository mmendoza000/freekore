...
class AppForm{
	...
	/**
	 * @desc It remuves the html &lt;form&gt; tag to use it under other appform.<br>
	...
	public function render($getCurrentUrl){
		...
		switch ($operation) {
			case 'new':
				...
				$rs .= __('<div class="message info">'.$this->Message_001.'</div>');
			...
			case 'duplic':
				...
				$this->btn_cancel_visible = TRUE;
				$this->update_nav_buttons('disable-all');
				$rs .= __('<div class="message info">'.$this->Message_007.'</div>');
				$rs .=$this-> PrintView('form');
				break;
			case 'save':
	...
	private function SaveRecord(){
		...
		foreach($this->DbRecord->form_fields as $f=>$arr ){
			...
			if(trim($post_value)==''){
				...
				if ($is_required){ echo fk_alert_message('<b>Error:</b> Required field',false); die();}
			}
	...
	private function PrintTemplateForm(){
		...
		if($this->remove_form==FALSE){
			$form .= '<form id="'. $this-> IdObject.'" name="'. $this-> IdObject.'" '.$multipart.' action="'.$action_sbmt.'" method="'.$method.'" class="form" >';
		}
