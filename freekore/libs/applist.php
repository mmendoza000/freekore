...
class AppList{
	...
	protected function createFilter() {
		...
		$this->filter = '
		<input type="hidden" name="page" id="page'.$this->formId.'" value="'.fk_post_get('page').'">
		<input type="hidden" name="sort" id="sort'.$this->formId.'" value="'.fk_post_get('sort').'">
		<input type="hidden" name="sorttyp" id="sorttyp'.$this->formId.'" value="'.fk_post_get('sorttyp').'">
		<div class="tbl-filter"><div class="input-append"><input type="text" name="p" id="p'.$this->formId.'" value="'.fk_post_get('p').'"><input type="submit" class="btn" value="Filtrar"></div></div>';
	...
	protected function PrintTemplateList(){
		...
		if(trim($this->currentUrl)!=''){
			...
			$this->btn_export = '<input type="button" class="btn print" value="'.__('Exportar a excel').'" onclick="window.open(\''.fk_link('FkMaster/export-list/'.encode('export').'/'.encode($this->currentUrl).'/').'\');" />';
		}else{
		...
		if(trim($this->currentUrl)!=''){
			...
			$this->btn_print = '<input type="button" class="btn print" value="'.__('Imprimir').'" onclick="window.open(\''.fk_link('FkMaster/export-list/'.encode('print').'/'.encode($this->currentUrl).'/').'\');" />';
		}else{
		...
		if(trim($this->currentUrl)!=''){
			...
			$this->btn_import = '<input type="button" class="btn import" value="'.__('Importar').'" onclick="window.open(\''.fk_link('FkMaster/import-list/'.encode('import').'/'.encode($this->currentUrl).'/').'\');" />';
		}else{
	...
	private function PrintCommonList(){
		...
		while($row = $this->db_queryapplist->next()){
			...
			if($line==0){
				...
				if($this->enable_buttons){$html .='<th style="width:100px;">&nbsp;</th>';}
	...
	private function add_page($i){
		...
		if($this->page==$i){
			$this->pagger .= ' <span>'.$i.'</span> ';
		}else{
			$this->pagger .= ' <a value="'.$i.'" href="?page='.$i.'">'.$i.'</a> ';
		}
	...
	protected function createPagger(){
		...
		if($add_first_page){	$this->add_page(1); $this->pagger .= '...';	}
		for($i=$ini;$i<=$fin;$i++){ $this->add_page($i); }
		if($add_last_page){ $this->pagger .= '...'; $this->add_page($this->tot_pages); }
	...
	public static function getOrderByProcesed($columns){
		...
		$orderBy = '';
		if(fk_post('sort')!=''){
			if(fk_post('sorttyp')==1){$ascDesc = ' desc ';}else{$ascDesc = ' asc ';}
			if(fk_post('sort')!=''){
				$orderBy = ' order by '.$columns[(fk_post('sort')-1)].' '.$ascDesc;
			}
